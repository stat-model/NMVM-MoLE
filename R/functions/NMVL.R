mix.NMVL.MoE.EM <- function(y, X, R, G = 2,
                                 max_iter = 200,
                                 tol = 1e-6,
                                 ridge = 1e-4,
                                 verbose = TRUE) {
  ...
  #------------------ Stable Bessel log ------------------#
  BesselK_log <- function(x, kappa) {
    log(besselK(x, kappa, expon.scaled = TRUE)) - x
  }
  
  #------------------ GH core (vectorized) ------------------#
  fGH.lind_vec <- function(x, tau, sigma2, kappa, psi) {
    chii <- (x^2) / sigma2
    psii <- psi + tau^2 / sigma2
    BesselK_log(sqrt(chii * psii), kappa - 0.5) -
      lgamma(kappa) - (kappa - 1)*log(2) +
      (0.5 - kappa)*(log(psii) - 0.5*log(chii*psii)) +
      kappa*log(psi) + x*tau/sigma2 - 0.5*log(2*pi*sigma2)
  }
  
  #------------------ NMVL log-density ------------------#
  logNMVL_vec <- function(x, sigma2, lambda, alpha2) {
    log1 <- log(alpha2/(1 + alpha2)) + fGH.lind_vec(x, lambda, sigma2, 1, 2*alpha2)
    log2 <- log(1/(1 + alpha2)) + fGH.lind_vec(x, lambda, sigma2, 2, 2*alpha2)
    m <- pmax(log1, log2)
  ...
  for (iter in 1:max_iter) {
    ...
    for (j in 1:G) {
      dens_mat[, j] <- exp(logNMVL_vec(y - mu_mat[, j],
                                       sigma2[j],
                                       lambda[j],
                                       alpha2[j]))
    }
    ...
    for (j in 1:G) {
      obj_a2 <- function(a2) {
        if (a2 <= 0) return(1e10)
        -sum(tau_ij[, j] *
               logNMVL_vec(y - X %*% beta[, j],
                           sigma2[j],
                           lambda[j],
                           a2))
      }
      opt <- optim(alpha2[j], obj_a2, method="L-BFGS-B", lower=1e-3, upper=50)
      alpha2[j] <- opt$par
    }
    ...
    opt <- optim(as.vector(alpha), loglik_alpha, method="BFGS", control=list(maxit=100))
