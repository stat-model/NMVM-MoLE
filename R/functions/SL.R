mix.SL.MoE.EM <- function(y, X, R, G = 2,
                               max_iter = 200,
                               tol = 1e-6,
                               ridge = 1e-4,
                               verbose = TRUE) {
  ...
  dSL_vec <- function(e, sigma2, lambda) {
    sigma <- sqrt(sigma2)
    tau <- sqrt(1 + (lambda / sigma)^2)
    logdens <- -log(2 * tau * sigma) - tau * abs(e)/sigma + lambda * e / sigma^2
    pmax(logdens, -700) |> exp()
  }
  ...
  for(iter in 1:max_iter){
    ...
    for(j in 1:G){
      dens_mat[, j] <- dSL_vec(y - mu_mat[, j], sigma2[j], lambda[j])
    }
